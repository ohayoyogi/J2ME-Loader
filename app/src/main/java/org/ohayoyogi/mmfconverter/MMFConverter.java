package org.ohayoyogi.mmfconverter;
...
public class MMFConverter {
    ...
    private class NoteMessage extends TrackEvent {
        ...
        void output(DataOutputStream dos, int currentTime) throws IOException {
            ...
            System.out.println(String.format("timestamp: %d, NoteMessage(channel=%d, note=%d, vel=%d, gateTime=%d)",
                    timestamp, channel, noteNumber, keyVelocity, gateTime));
        }
    ...
    private class ControlChange extends TrackEvent {
        ...
        void output(DataOutputStream dos, int currentTime) throws IOException {
            // writeVariableLengthValue(timestamp - currentTime, dos);
            // dos.writeByte(0x9F & ( 0xF0 | channel));
            // dos.writeByte(controlNumber);
            // dos.writeByte(controlValue);
            System.out.println(String.format("timestamp: %d, ControlChange(channel=%d, no=%d, val=%d)", timestamp,
    ...
    private class ProgramChange extends TrackEvent {
        ...
        void output(DataOutputStream dos, int currentTime) throws IOException {
            ...
            System.out.println(String.format("timestamp: %d, ProgramChange(channel=%d, no=%d)", timestamp, channel,
                    programNumber));
    ...
    void convertToMDI(File file) throws Exception {
        byte[] data = new byte[100000];
        int readSize = 0;
        int fileSize = 0;
        try (FileInputStream fis = new FileInputStream(file)) {
            while ((readSize = fis.read(data, fileSize, 1000)) > 0) {
                fileSize += readSize;
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    ArrayList<TrackEvent> parseSequenceDataChunk(byte[] data) throws Exception {
        ArrayList<TrackEvent> result = new ArrayList<TrackEvent>();
        Map<Integer, Integer> mapVel = new HashMap<Integer, Integer>();
        ...
        try (ByteArrayInputStream is = new ByteArrayInputStream(data); DataInputStream dis = new DataInputStream(is)) {
            ...
            while (dis.read(firstByte, 0, 1) == 1) {
                ...
                globalTime += duration;
                System.out.println("duration: " + duration + " eventHeader: " + toHex(eventHeader));
    ...
    void parseMTR(byte data[], DataOutputStream dos) throws Exception {
        try (ByteArrayInputStream is = new ByteArrayInputStream(data); DataInputStream dis = new DataInputStream(is)) {
            ...
            System.out.println("timeBaseG: " + timeBaseG);
            ...
            while (dis.read(chunkId, 0, 4) == 4) {
                ...
                if (Arrays.equals(chunkId, "Mtsq".getBytes())) {
                    ...
                    try (ByteArrayOutputStream os = new ByteArrayOutputStream();
                        ...
                        ddos.write(new byte[]{(byte) 0xFF, 0x20, 0x00});
